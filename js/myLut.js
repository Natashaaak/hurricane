...
class Lut {
     ...
     */
    createCanvas ( min, max, isoMin, isoMax, isoStep, contours ) {

        const canvas = document.createElement( 'canvas' );
        canvas.width = 1;
        canvas.height = this.n;

        this.updateCanvas( canvas, min, max, isoMin, isoMax, isoStep, contours );
     ...
     */
    updateCanvas( canvas, min, max, isoMin, isoMax, isoStep, contours ) {
        ...
        // draw contours
        if ( contours ) {
            for ( let i = isoMin; i <= isoMax + isoStep / 2; i += isoStep ) {
                if ( i <= max && i >= min ) {
                    let pos = ( i - min ) / ( max - min );
                    pos = Math.round ( this.n * pos );
                    pos = this.n - pos;
